...
We are going to consider a set of I.I.D. \(X_1,\dots,X_n\) representing
each an exoplanet orbital period from a dataset of exoplanet observed
via the Kepler telescope, and we are going to study their distribution.
...
shape: the log-normal distribution \(LogN(\mu,\sigma^2)\) and the gamma
distribution \(Gamma(k,\theta)\). We will check which better fits our
data.
...
Let \(\nu\) be the set of parameters, \(m\in\{1,2\}\) the parameter
identifing the model where \(1\) stand for the log-normal distribution
and \(2\) for the gamma distribution. We also take \(p(\cdot|m,\nu)\)
the distribution correspondent to one of the two model \(m\). If
\(\nu_1,\dots,\nu_l\) are I.I.D. from the posteriori distribution
\(\pi(\nu|x_1,\dots,x_n,m)\) then our posteriori predictive distribution
...
We already have the \(\nu_i\) as they are produced in the MCMC,
therefore truncating the series we get an approximation of our posterior
...
distributed observation \(y = (y_1,\dots,y_n)\) as
\[\tilde{X^2}(y;\mu,\sigma^2) = \sum_{i=1}^n\frac{(y_i - \mu)^2}{\sigma^2}\]
Let’s remark that if \(\mu\) and \(\sigma^2\) are the mean and standard
deviation of the distribution from where we sample the \(y_i\) then
\(X^2\) is distributed as a \(\chi^2_n\) distribution.

As our variable is log-normal distributed we need to adapt this
discrepancy function, considering \(\ln(y_i)\) instead of \(y_i\). In
our case we will use
\[X^2(y;\mu,\sigma^2) = \sum_{i=1}^n\frac{(\ln(y_i) - \mu)^2}{\sigma^2}\]

Let us write as before \(\nu = \mu,\sigma^2\). In our specific case we
will define the posterior predictive p-value
...
parameters, but their distribution \(\pi(\cdot|x_1\dots,x_n)\) and
therefore we consider the above integral averaging all the possible
\(X^2(\cdot|\nu)\) functions.
...
are well known: \[\hat\mu = \frac{\sum_i \ln x_i}{n}\]
...
fit our data. We will compare \(LogN(\mu_{MLE},\sigma^2_{MLE})\) with
the predictive posteriori distribution \(m(x|x_1,\dots,x_n)\) obtained
from the MCMC based on the log-normal priori.
...
again use the \(X^2\) function defined above. We already computed the
p-value of our two models for this test:
